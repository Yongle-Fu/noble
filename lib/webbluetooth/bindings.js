...
NobleBindings.prototype.init = function (ble) {
  ...
  nextTick(async () => {
    ...
    debug(`> Bluetooth is ${isBluetoothAvailable ? 'available' : 'unavailable'}`);
    self.emit('stateChange', isBluetoothAvailable ? 'poweredOn' : 'poweredOff');
    if ('onavailabilitychanged' in navigator.bluetooth) {
      navigator.bluetooth.addEventListener('availabilitychanged', function (isBluetoothAvailable) {
        self.emit('stateChange', isBluetoothAvailable ? 'poweredOn' : 'poweredOff');
      });
    }
  });
...
NobleBindings.prototype.startScanning = function (options, allowDuplicates) {
  ...
  if (options.name) {
    ...
    filterList.push({ services: dashedUuids });
  }

  const request = options.acceptAllDevices ? { acceptAllDevices: true } : { filters: filterList };
  request.optionalServices = ['battery_service', 'device_information', ...dashedUuids];
  debug('request.optionalServices', request.optionalServices);
    ...
    .then(device => {
      ...
      if (device) {
        ...
        self._peripherals[address] = {
          ...
          serviceUuids: request.optionalServices
        };
        if (device.adData) {
